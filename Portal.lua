	...
	do
		...
		function Library:Window(Options)
			local Window = {
				...
				Name = Options.name or Options.Name or "DH Enhanced";
			};
			--
			local DHEnhancedGUI = Instance.new("ScreenGui")
			...
			DHEnhancedGUI.Name = "DHEnhancedGUI"
			DHEnhancedGUI.Parent = game.CoreGui
			DHEnhancedGUI.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
			Library.ScreenGUI = DHEnhancedGUI
            ...
            if not isfolder("DHEnhanced") then 
                makefolder("DHEnhanced")
            end

            if not isfolder("DHEnhanced/configs") then 
                makefolder("DHEnhanced/configs")
            end

            if not isfile("DHEnhanced/configs/Default.cfg") then 
                writefile("DHEnhanced/configs/Default.cfg","")
            end

			-- Config
			Library:Connection(Save.MouseButton1Down, function()
				writefile("DHEnhanced/configs/Default.cfg", Library:GetConfig())
			...
			Library:Connection(Load.MouseButton1Down, function()
				if isfile("DHEnhanced/configs/Default.cfg") then
					Library:LoadConfig(readfile("DHEnhanced/configs/Default.cfg"))
				end
...
local Notification = Value and "Enabled" or "Disabled"
